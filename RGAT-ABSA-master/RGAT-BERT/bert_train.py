...
parser.add_argument("--model", type=str, default="rgat", help="model to use, (std, GAT, SGAT)")
...
# set random seed 设置随机种子，以确保在每次运行代码时生成的随机数相同。
...
# load vocab 加载预先训练好的词汇表文件，并计算每个词汇表的大小
print("Loading vocab...")
...
pos_vocab = Vocab.load_vocab(args.vocab_dir + "/vocab_pos.vocab")  # POS 表示词性
dep_vocab = Vocab.load_vocab(args.vocab_dir + "/vocab_dep.vocab")  # deprel 表示依存关系
pol_vocab = Vocab.load_vocab(args.vocab_dir + "/vocab_pol.vocab")  # polarity 表示情感极性
...
helper.ensure_dir(model_save_dir, verbose=True)  # 确保指定的目录存在。如果目录不存在，则会创建该目录。参数verbose=True表示在创建目录时打印一条消息，以便在控制台中显示目录已创建
...
def _totally_parameters(model):  # 计算模型中所有参数的元素数量，并将它们相加得到总元素数量 p.nelement()计算一个参数 p 中的所有元素的数量
...
print("trainer:{}".format(trainer.model))
